(function (V, W, $) {
    V.registScript(function (path, vm) {
		...
		_.fill = function () { return { value: _.node.get_html ? V.encHtml(_.node.get_html().replace(/</g,'&lt;').replace(/>/g,'&gt;')) : "" } };
		_.render = function (data) {
			var ret = V.merge({},data);
            ...
            V.forC(ret, function (k, v) {
                switch (k.toLowerCase()) {
					...
					case 'images':
						...
						break;
					case 'value':
						if(_.editor) {V.once(function(){_.node.set_text(V.decHtml(v));},100);} else render = true;
						break;
                }
            },function () {
				if(render && !_.editor){				
					...
					_.node.htmlbox(ret);
					if(data.value){
						_.node.set_text(V.decHtml(data.value));
					}
				}
				delete data.value;
				__.render(data);
