        ...
        var _ = this,
            ...
            V.inherit.apply(_, [W.Control, [path, vm || {
                ...
                addItem: function(val) {
                    _.ni.execute('template', _.vm.data.addTemplateName, V.merge(_.vm.data.params, _.vm.data.value, val), function(res) {
                        if (res.hasData()) {
                ...
                removeItem: function(val) {
                    _.ni.execute('template', _.vm.data.removeTemplateName, V.merge(_.vm.data.params, _.vm.data.value, val), function(res) {
                        if (res.hasData()) {
                            var ret = res.last()[0][0];
                            if (ret.count > 0) {
                ...
                refresh: function() {
                    _.ni.execute('template', _.vm.data.refreshLeftTemplateName, V.merge(_.vm.data.params, _.vm.data.value), function(res) {
                        if (res.hasData()) {
                            _.vms.lstLeft.update({ values: res.last()[0] });
                        }
                    });
                    _.ni.execute('template', _.vm.data.refreshRightTemplateName, V.merge(_.vm.data.params, _.vm.data.value), function(res) {
                        if (res.hasData()) {
