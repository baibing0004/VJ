...
$.fn.htmlbox=function(options){
		...
		for(var k=0;k<d.toolbars.length;k++){
			...
			for(var i=0;i<(toolbar.length);i++){
	            ...
	            // END: Custom button
					...
					h += "</select></td>";
				}else if(toolbar[i]==="control"){
				    h += "<td valign='middle' align='center'><select id='"+d.id+"_control' onchange='global_hb[\""+d.id+"\"].cmd(\"control\",this.options[this.selectedIndex].value);this.options[0].selected=\"true\";' style='font-size:12px;'><option value='' selected>- 标签 -</option><option value='text'>输入框</option><option value='select'>下拉框</option></select></td>";
				}
				// Commands
				var cmds = getLanguage(d.language);
	...
	this.cmd = function(cmd,arg1){
	   ...
	   if(cmd==="code"){
		   ...
		   switch(arg1){
			   case 'text':
			     text+='<input type="text" class="control-text" />';
				 break;
			   case 'select':
			     text+='<select style="width:100px;"><option value="1">1</option><option value="2">2</option><option value="3">3</option></select>';
			       break;
		   }
		       this.set_text(text);
			   $("#"+d.id).focus();
