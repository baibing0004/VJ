            ...
            V.forC(_.events, function (k, v) {
                ...
                UE.registerUI('input', function (editor, uiName) {	//注册按钮执行时的command命令，使用命令默认就会带有回退操作
                    ...
                    var btn = new UE.ui.Button({
                        ...
                        cssRules: 'background-position: -242px -20px;',
                        //点击时执行的命令
                        onclick: function () {
                ...
                UE.registerUI('textarea', function (editor, uiName) {	//注册按钮执行时的command命令，使用命令默认就会带有回退操作
                    ...
                    var btn = new UE.ui.Button({
                        ...
                        title: '新建大文本框',
                        //添加额外样式，指定icon图标，这里默认使用一个重复的icon
                        cssRules: 'background-position: -238px -75px;',
                        //点击时执行的命令
                        onclick: function () {
                            //这里可以不用执行命令,做你自己的操作也可
                            editor.execCommand(uiName);
                ...
                UE.registerUI('area', function (editor, uiName) {	//注册按钮执行时的command命令，使用命令默认就会带有回退操作
                    editor.registerCommand(uiName, {
                        execCommand: function () {
                            editor.execCommand('inserthtml', V.format("<img src='" + editor.options.UEDITOR_HOME_URL + "dialogs/input/inputarea.png' id='{id}' _type='area' _data='{data}'/>", { id: V.random(), data: "{}" }));
                        }
                    });
                    //创建一个button
                    var btn = new UE.ui.Button({
                        ...
                        title: '新建列表',
                        //添加额外样式，指定icon图标，这里默认使用一个重复的icon
                        cssRules: 'background-position:-522px 1px;',
                        //点击时执行的命令
                        onclick: function () {
                            //这里可以不用执行命令,做你自己的操作也可
                            editor.execCommand(uiName);
                ...
                if (render) {
                    ...
                    editor.ready(function () {
                        _.editor = editor;
                        {
                            ...
                            _.editor.addListener('contentChange', function () { _.call('change'); });
                        ...
                        }
                        {
                            //添加对应对话框 需要在all.min.js中添加对象
                            //editor.options.iframeUrlMap["inputtext"]='~/dialog/input/inputtext.htm';
                            var dialog3 = new baidu.editor.ui.Dialog({
                                iframeUrl: editor.ui.mapUrl('~/dialogs/input/inputarea.htm'),
                                editor: editor,
                                className: 'edui-for-snapscreen',
                                title: '新建列表',
                                buttons: [
                            ...
                            editor.ui._dialogs["inputarea"] = dialog3;
                            editor.ui._dialogs["inputarea"].render();
                            $('#' + dialog3.id + '_body').width(700 - (_.vm.data.inner ? 100 : 0)).height(570 - (_.vm.data.inner ? 200 : 0));
                            $('#' + dialog3.id + '_content').width(690 - (_.vm.data.inner ? 100 : 0)).height(492 - (_.vm.data.inner ? 200 : 0));
                            dialog3.addListener('show', function () {
                                $('#' + dialog3.id + '_body').width(700 - (_.vm.data.inner ? 100 : 0)).height(570 - (_.vm.data.inner ? 200 : 0));
                                $('#' + dialog3.id + '_content').width(690 - (_.vm.data.inner ? 100 : 0)).height(492 - (_.vm.data.inner ? 200 : 0));
                            });
                        }
