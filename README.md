...
* VJ本身提供的不是一个瑰丽的UI框架，甚至特别绚丽的单一界面功能,而是更加方便地在不改动原有代码的情况下应用各种炫控件和新逻辑的可能：
	...
	* 总线架构模式就是CVVM基于分控件包干的设计思想下提出异步控件调用模式，其保证控件之间在异步加载的不同步情况下，调用的顺序和逻辑是正确的，甚至控件的升级和替换，加载与不加载的情况下都能保证主业务逻辑和顺序是正确的。其总体上分为集中调用和分布调用两种不同的常用调用模式，在VJ中使用regist/callEvent和regist/callCommand两种调用方式哪怕call在regist之前也能保证 正常的调用。
    * 它首先改变的是 **你对JS的看法**；
